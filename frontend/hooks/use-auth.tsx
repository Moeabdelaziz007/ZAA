...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const registerMutation = useMutation({
    ...
    onSuccess: (user: SelectUser) => {
      ...
      toast({
        ...
        description: `Welcome to StayChill, ${user.displayName || user.username}!`,
      });
    },
    onError: (error: Error) => {
      toast({
  ...
  // This is a default implementation of the guest login function
  // It's used as a fallback in case the context value is not properly set
  const contextValue = {
    user: user ?? null,
    isLoading,
    error,
